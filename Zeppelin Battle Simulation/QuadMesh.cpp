...
QuadMesh::QuadMesh(int maxMeshSize, float meshDim):groundTextureID(0)
{
	minMeshSize =1;
	numVertices = 0;
	vertices = NULL;
...
void QuadMesh::UpdateMesh(double elapsedTime)
{
    // Iterate through all vertices and apply a simple wave function
    for (int i = 0; i < (maxMeshSize + 1) * (maxMeshSize + 1); i++)
    {
        float frequency = 1.0f;  // Adjust the frequency of the wave
        float amplitude = 0.5f;  // Adjust the amplitude of the wave

        // Modify the Y-coordinate of each vertex based on a sine wave
        vertices[i].position.y = amplitude * sin(frequency * elapsedTime + vertices[i].position.x);
    }

    // Recompute normals after updating vertex positions
    ComputeNormals();
}
