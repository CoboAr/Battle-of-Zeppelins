struct MeshVertex
{
    ...
    float waveDisplacement;  // New parameter for wave effect
};
...
public:
    ...
    void DrawMesh(int meshSize);  // Add the elapsedTime parameter
    void UpdateMesh();
	void SetMaterial(VECTOR3D ambient, VECTOR3D diffuse, VECTOR3D specular, double shininess);
	void ComputeNormals();
    
    void UpdateMesh(double elapsedTime);
    
    // Set texture ID
        void SetTextureID(GLuint texID) {
            groundTextureID = texID;
        }
